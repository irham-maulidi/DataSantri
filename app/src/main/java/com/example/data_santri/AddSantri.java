...
public class AddSantri extends AppCompatActivity {

    @BindView(R.id.edtNamaSantri)
    ...
    private void saveData() {
        ...
        // Membuat penampung dengan membaut object SantriModel
        ModelSantri santriModel = new ModelSantri();

        // kita masukkan data ke dalam santriModel
        santriModel.setId_kelas(id_kelas);
        santriModel.setNama_siswa(namaSiswa);
        santriModel.setAsal(asal);
        santriModel.setUmur(umur);
        santriModel.setJenis_kelamin(jenis_kelamin);
        santriModel.setEmail(email);

        // Kita lakukan operasi insert
        santriDatabase.kelasDao().insertSiswa(santriModel);
    }
